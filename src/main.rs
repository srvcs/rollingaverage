use srvcs_rollingaverage::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-rollingaverage listening");
    let deps = Deps {
        movingaverage_url: cfg.movingaverage_url.clone(),
    };
